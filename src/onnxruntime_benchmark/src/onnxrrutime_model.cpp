...
void ONNXModel::fill_inputs_outputs_info() {
    ...
    for (size_t i = 0; i < session->GetInputCount(); ++i) {
        ...
        io.input_data_precisions.push_back(get_data_precision(type));
...
std::vector<ONNXTensorDescr> ONNXModel::get_input_tensors_info() const{
    ...
    for (int i = 0; i < io.input_names.size(); ++i) {
        input_tensors_info.push_back({std::string(io.input_names[i]), io.input_shapes[i], "", io.input_data_precisions[i],
...
void ONNXModel::run(const std::vector<Ort::Value>& input_tensors) const {
    auto output_tensors = session->Run(Ort::RunOptions{nullptr}, io.input_names.data(), input_tensors.data(),
        io.input_names.size(), io.output_names.data(), io.output_names.size());
