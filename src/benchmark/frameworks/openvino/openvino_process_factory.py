...
def create_process(test, executor, log, cpp_benchmarks_path=None):
    ...
    if mode == 'ovbenchmark_cpp_latency':
        return OpenVINOBenchmarkCppProcess(test, executor, log, cpp_benchmarks_path, 'latency')
    if mode == 'ovbenchmark_cpp_throughput':
        return OpenVINOBenchmarkCppProcess(test, executor, log, cpp_benchmarks_path, 'throughput')
